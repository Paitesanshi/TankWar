...
public class NetClient {
    ...
    public void connect(String ip){
        ...
        try {
            ...
            tc.getMyTank().setGood((id & 1) == 0 ? true : false);//根据坦克的id号分配阵营，偶数好，奇数坏
        ...
        TankNewMsg msg = new TankNewMsg(tc.getMyTank());//发送创建坦克的消息
        send(msg);
    }
    ...
    public class UDPThread implements Runnable{
        ...
        public void run() {
            while(ds !=null ){
                DatagramPacket dp = new DatagramPacket(buf, buf.length);//创建接受UDP数据包
                try{
                    ds.receive(dp);
                    parse(dp);
                } catch (IOException e) {
    ...
    public void sendClientDisconnectMsg(){
        ...
        byte[] buf = baos.toByteArray();//获取字节数组缓冲区中数据
        try{
            DatagramPacket dp = new DatagramPacket(buf, buf.length, new InetSocketAddress(serverIP, TANK_DEAD_UDP_PORT));//断开连接后坦克死亡
            ds.send(dp);
        } catch (IOException e) {
            e.printStackTrace();
        }
